...
void spi_write(uint8_t *data, uint8_t len)
{
    ...
    for ( x = 0; x < len; x++)
    {
        for (i = 7; i >= 0; i--)
        {
            ...
            GPIO_WritePin(gpio->MOSI_PORT, gpio->MOSI_PIN, (data[x] & (1 << i)));
            //delay_ms(1);
            GPIO_WritePin(gpio->CLK_PORT, gpio->CLK_PIN, 1);
            //delay_ms(1);
            GPIO_WritePin(gpio->CLK_PORT, gpio->CLK_PIN, 0);
        }
    }
...
void spi_read(uint8_t *buffer, uint8_t len)
{
    ...
    for ( x = 0; x < len; x++)
    {
        ...
        for ( i = 7; i >= 0; i--)
        {
            ...
            GPIO_WritePin(gpio->CLK_PORT, gpio->CLK_PIN, 1);
            delay_ms(1);
            buffer[x] |= GPIO_ReadPin(gpio->MISO_PORT, gpio->MISO_PIN);
            delay_ms(1);
            GPIO_WritePin(gpio->CLK_PORT, gpio->CLK_PIN, 0);
        }
    }
...
void MAX31865_read(uint8_t addr, uint8_t *buffer, uint8_t len)
{
  addr &= ~MAX31865_READ;                                      // Force read bit on address
    ...
    GPIO_WritePin(gpio->CE_PORT, gpio->CE_PIN, 0);          // Enable CE

    spi_write(&addr, 1);                                        // Write addr
    spi_read(buffer, len);                                      // Read data

    GPIO_WritePin(gpio->CE_PORT, gpio->CE_PIN, 1);          // Disable CE
}
...
void MAX31865_init(MAX31865_GPIO *max_gpio, uint8_t wires)
{
    ...
    GPIO_WritePin(gpio->CE_PORT, gpio->CE_PIN, 1);
    GPIO_WritePin(gpio->CLK_PORT, gpio->CLK_PIN, 1);
    GPIO_WritePin(gpio->MOSI_PORT, gpio->MOSI_PIN, 1);
